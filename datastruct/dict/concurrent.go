...
// 原生dict解决大表阻塞问题：用的是渐进式rehash，
// rehash过程中每次操作都rehash一部分数据，直到rehash完成
// 这里的实现式分段式锁，分段锁的粒度是分段，
// 分段内用普通的map实现，分段之间用锁隔离，减少锁竞争，
// 提高并发性能
//通配符正则化匹配
import (
	"math"
	"math/rand"
	"my-redis/lib/wildcard"
	"sort"
	"sync"
	"sync/atomic"
	"time"
...
// 创建给定数量的高性能并发安全分段锁map（concurrentDict）
...
// FNV hash ==redis 没有使用，redis在6之后使用了SipHash算法，性能更好，安全性更高
...
// 用在不加锁的地方，比如在Set方法中，先获取锁，再调用GetWithLock方法获取值
