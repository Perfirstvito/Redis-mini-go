...
import (
	...
	SortedSet "my-redis/datastruct/sortedset"
...
func (server *Server) loadRdbFile() error {
	...
	if err != nil {
		return fmt.Errorf("open rdb file %q failed: %w", config.Properties.RDBFilename, err)
	}
	defer func() {
		_ = rdbFile.Close()
	}()
	...
	if err != nil {
		return fmt.Errorf("load rdb file %q failed: %w", config.Properties.RDBFilename, err)
	}
	return nil
}
